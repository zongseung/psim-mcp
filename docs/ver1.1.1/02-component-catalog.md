...
- [x] 모든 부품에 핀 정의 추가 (40+ 부품, 각 핀 목록 포함)
- [x] PinDef, ParamDef 데이터클래스 정의
- [x] PSIM element type 필드 추가 (값은 Windows 확인 후 채움)
- [x] get_component(), validate_kind(), validate_pin(), get_default_params() 헬퍼 함수
- [x] 기존 테스트 통과
- [x] get_component_library MCP 도구로 Claude에게 부품 정보 제공 (14번째 도구)
- [x] connection 검증에서 catalog 핀 정의 참조 (validate_connections)
- [ ] svg_renderer, ascii_renderer가 catalog 핀 정의를 참조 (향후 통합)
- [ ] bridge_script가 kind → psim_element_type 변환 사용 (Windows 검증 후)
