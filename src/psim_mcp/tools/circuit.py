...
def register_tools(mcp, service=None):
    ...
    @mcp.tool(
        description=(
            ...
            "       float Kp = 0.02f;                          // ← 사용자 spec 그대로 (x3 무시)\n"
            "       float Ti = 0.001f;                         // ← 사용자 spec 그대로 (x4 무시)\n"
            "       float Fsamp_eff = 12500.0f;                // ← 아래 'dt 가이드' 참조, x5 직접 쓰지 말 것\n"
            "       if (ap_start == 1 && Flag == 0) {\n"
            "           Flag = 1;\n"
            "           float I_ref   = x1 / counts_per_A;     // counts → A\n"
            "           float I_sense = x2 / counts_per_A;\n"
            "           float err     = I_ref - I_sense;       // A 단위\n"
            "           float dt      = 1.0f / Fsamp_eff;      // 하드코딩한 effective rate 사용\n"
            "           float prop    = Kp * err;              // dimensionless\n"
            "           float u       = prop + (Kp / Ti) * (integ + err * dt);\n"
            "           if (u >= 1.0f) { u = 1.0f; integ = (1.0f - prop) * Ti / Kp; }       // back-calc anti-windup\n"
            ...
            "    🚨 **CRITICAL — dt 가이드 (`dt = 1.0f/x5` 무비판 사용 금지)**: 사용자가 Kp/Ti를 물리 단위로 줬으면 dt도 사용자 의도의 effective sample rate에 맞춰 하드코딩해야 합니다. chassis CONSTANT의 `Fsamp` (=x5)는 *PWM 캐리어 주파수* (예: 200kHz)지 ap_start 발화 주기가 아닙니다. 검증된 chassis의 ap_start 실측 effective rate는 **12.5kHz** (buck c-block chassis의 prescaler x8=8 적용 후 100kHz/8). `dt = 1/x5 = 5µs` 그대로 쓰면 의도보다 **16배 약하게 적분** → IL이 ref 추종 못하고 Vo가 13.6V 같은 미달 운영점에 stuck (이 세션 v3 실험에서 -9% tracking error 재현). 안전한 기본값: ``float Fsamp_eff = 12500.0f; float dt = 1.0f/Fsamp_eff;``. 다른 chassis 쓸 땐 stock 동작점에서 ap_start 신호 주기를 SimView로 직접 측정 후 하드코딩.\n"
            "    검증 기준 (analyze_existing의 mean 메트릭): IL_digital이 IL_ref ±10 counts 안에 들고 Vo가 목표 ±5% 안에 들면 OK. 추종 안 되면 dt 가이드 먼저 점검.\n"
