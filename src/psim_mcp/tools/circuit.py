...
def register_tools(mcp, service=None):
    ...
    @mcp.tool(
        description=(
            ...
            "       float I_ref = (x1 / 4095.0f) * I_max;     // counts → A\n"
            "       float I_sense = (x2 / 4095.0f) * I_max;\n"
            "       float err = I_ref - I_sense;              // A 단위 error\n"
            "       static float integ = 0;\n"
            "       float dt = 1.0f / x5;                     // x5 = Fsamp\n"
            "       integ += err * dt;\n"
            "       float u = x3 * err + (x3 / x4) * integ;   // A → 0..1 normalized\n"
            "       if (u > 1.0f) u = 1.0f;\n"
            "       if (u < 0.0f) u = 0.0f;\n"
            "       y1 = u * duty_max;                        // normalized → duty word\n"
            "    이 변환을 안 하고 raw counts에 Kp 곱하면 1000~2000 counts × 0.2 = duty 200~400로 즉시 포화 → 시뮬 발산(IL ±130A, Vo 84V)함.\n"
