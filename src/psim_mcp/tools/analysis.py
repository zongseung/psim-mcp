...
def register_tools(mcp, service=None, adapter=None):
    ...
    @mcp.tool(
        description=(
            ...
            "  - 개루프 buck (자체 합성 회로): topology='buck' (default) — 신호는 V(Vout), I(L1)\n"
            "  - **폐루프 buck (preview_circuit에 closed_loop=True로 만든 chassis 회로): topology='buck_closed_loop' 반드시 명시**\n"
            "    이 chassis의 실제 VP 프로브명은 ``Vo``, ``I(L1)``, ``IL_sensed_analog_by_adc_gain``, "
            "``IL_ref``, ``Duty_cycle``로 canonical buck과 다릅니다. ``topology='buck'``으로 호출하면 "
            "신호명 미스매치로 ``metrics``가 전부 ``signal 'V(Vout)' not found`` 에러가 되고, "
            "호출자가 빈 메트릭에서 raw last_value를 잡아 가짜 진단(예: \"ADC cal 250 cnt/A\")을 만들어 "
            "낼 위험이 큼 — 응답의 ``available_signals``를 먼저 확인하고 chassis 신호명이 보이면 "
            "반드시 ``buck_closed_loop``로 재호출하세요.\n"
            "  - 폐루프 boost / 기타: 별도 등록 전까지 ``available_signals``로 신호명 보고 직접 graph_file 분석.\n\n"
