"""Boost converter synthesizer — creates CircuitGraph from requirements.

Non-isolated step-up DC-DC converter. NO position/direction/ports.
"""
...
def synthesize_boost(requirements: dict) -> CircuitGraph:
    """Synthesize a boost converter CircuitGraph from requirements."""
    ...
    nets = [
        make_net("net_vin_l", ["V1.positive", "L1.pin1"], role="input_positive"),
        make_net("net_l_sw_d", ["L1.pin2", "SW1.drain", "D1.anode"], role="switch_node"),
        ...
        make_net(
            "net_gnd",
            ["V1.negative", "GND1.pin1", "SW1.source", "C1.negative", "Vout.pin2"],
            role="ground",
        ),
    ]
    ...
    blocks = [
        ...
        make_block("boost_stage", "switching", role="switching", component_ids=["L1", "SW1", "G1", "D1"]),
        make_block("output_filter", "filter", role="output", component_ids=["C1", "Vout"]),
    ]

    traces = [
        make_trace("design_formula", "duty", round(duty, 6), rationale="D = 1 - Vin/Vout"),
        make_trace("design_formula", "inductance", round(inductance, 9), rationale="L = Vin*D/(fsw*dI)"),
        make_trace("design_formula", "capacitance", round(capacitance, 9), rationale="C = Iout*D/(fsw*dVr*Vout)"),
    ]
