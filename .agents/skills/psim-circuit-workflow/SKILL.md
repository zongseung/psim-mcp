...
6. Sweeps and comparison — one-dimensional scan: `sweep_parameter` (bounded step count, early-stops after 3 consecutive failures). Judgment-driven tuning: loop `set_parameter → run_simulation → analyze_existing` one step at a time. Before/after evidence: analyze each result with `analyze_existing` and compare the returned metrics.
